...
class Expression:
    ...
    def __init__(self, raw):
        ...
        ExpressionAsserter.assertRawIsValid(raw)
        ...
        self.parsed = None
        self.__createParsedExpression()
        
        self.varCount = len(self.vars.keys())
        
    def extractVariables(self):
        for index in self.LRANGE:
            char = self.__getCharAt(index)
            self.__addVariableIfValid(char, index)

    def __addVariableIfValid(self, char, index):
        if (ExpressionAsserter.isInAlphabet(char)):
            self.__addVariablePosition(char, index)

    def __getCharAt(self, index):
        return self.raw[index]

    def __addVariablePosition(self, char, index):
        try:
            self.vars[char].append(index)
        except:
            self.vars[char] = [index]

    def __createParsedExpression(self):
        # initialize and compute parsed
        self.parsed = Expression.__getNextExpression(self.raw)
    ...
    @staticmethod
    def __getNextExpression(raw_substring):
        ...
        while index < L:
            ...
            if (ExpressionAsserter.isInAlphabet(char)):
            ...
            elif (ExpressionAsserter.isOpeningParenthesis(char)):
                # find the closing index and get the substring, 
                # from which we find the parsed expression 
                closing_index = Expression.findClosingParenthesisFrom(raw_substring, index)
                sub_raw_substring = raw_substring[index+1:closing_index]
                # append the expression in the parentheses to the stack
                expression_stack.append(Expression.__getNextExpression(sub_raw_substring))
                # set the index to the closing parenthesis
                index = closing_index
            elif (ExpressionAsserter.isOperator(char)):
                ...
                operation = Expression.__getOperation(operation, char)   
        ...
        Expression.__assertOperationIsValid(operation, expression_stack, raw_substring)
    ...
    @staticmethod
    def __getOperation(operation, char, raw = None):
        if ord(char) == operation:
            return operation
        if (operation != None):
            Expression.__raiseSyntaxOperationError(raw)
        return ord(char)

    @staticmethod
    def __raiseSyntaxOperationError(raw = None):
        raw_string = " "
        if (raw != None):
            raw_string = " (" + str(raw) + ") "
    ...
    @staticmethod
    def __assertOperationIsValid(operation, expression_stack, raw_substring):
        ...
        Expression.__assertNOTOperationCase(operation, expression_stack, raw_substring)
        # Raise a SyntaxError in case the * or + operation is 
        # done on only 1 Expression or Value
        Expression.__assertOtherOperationsCase(operation, expression_stack, raw_substring)

    @staticmethod
    def __assertNOTOperationCase(operation, expression_stack, raw_substring):
        ...
        if is_stack_length_valid_for_NOT and is_NOT_operation:
            Expression.__raiseSyntaxNOTOperationError(raw_substring)

    @staticmethod
    def __raiseSyntaxNOTOperationError(_raw = None):
    ...
    @staticmethod
    def __assertOtherOperationsCase(operation, expression_stack, raw_substring):
        ...
        if is_stack_length_valid_for_AND_or_OR and is_AND_or_OR_operation:
            Expression.__raiseSyntaxOtherOperationsError(raw_substring)
    
    @staticmethod
    def __raiseSyntaxOtherOperationsError(_raw = None):
        raw_string = raw_substring = " " if (_raw == None) else " (" + str(_raw) + ") "
    ...
    @staticmethod
    def findClosingParenthesisFrom(raw_substring, index):
        ...
        assert ExpressionAsserter.isOpeningParenthesis(opening), error_string
